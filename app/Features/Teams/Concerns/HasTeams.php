...
namespace App\Features\Teams\Concerns;

use App\Features\Teams\Data\TeamPermissions;
use App\Features\Teams\Data\UserTeam;
...
trait HasTeams
{
    ...
    {
        if (! $this->belongsToTeam($team) && ! $this->is_admin) {
            return false;
        }
     ...
     */
    public function toUserTeams(bool $includeCurrent = false, bool $onlyActive = true): Collection
    {
        $query = $this->is_admin
            ? Team::query()
            : $this->teams();
        ...
        return $query
            ->when($onlyActive, fn ($query) => $query->where('teams.is_active', true))
    ...
    public function toUserTeam(Team $team): UserTeam
    {
        ...
        if ($this->is_admin && $role === null) {
            return new UserTeam(
                ...
                roleLabel: TeamRole::Admin->label(),
